...
\begin{enumerate}
    \item Gather requirements to inform the design of the imaging spectrometer.
    \item Design the imaging spectrometer, making use of commercially available components that are either
    already present in the laboratory or can be procured from companies such as Thorlabs.
    \item Construct and align the imaging spectrometer.
    \item Characterise the imaging spectrometer using spectral calibration lamps.
    \item Write a user interface for the imaging spectrometer for data acquisition and analysis.
    \item Test the imaging spectrometer on a laser pulse with a known amount of spatial chirp.
\end{enumerate}
...
%
% NOTE for the Optical Design and Analysis Pipeline paragraphs specifically:
% flag the forward reference between them -- Chapter 2 states the chosen
% focal lengths descriptively and defers their quantitative justification to
% Section~\ref{sec:pipeline-focal-length-tradeoff} in Chapter 4, once the
% noise model and dispersion fit exist to support the error-propagation
% argument. Say this explicitly in the outline so it reads as deliberate
