...
flat field, bad-pixel map, conversion gain, spatial scale factor), with no
intermediate review page. Success builds a CalibrationBundle and emits
calibration_ready right away; failure (any of baseline/flat-field/bad-
pixel-map/conversion-gain missing -- spatial's scale factor always falls
back to a physically valid default, so it alone missing doesn't count) or
...
"Continue" path). Note this method does not read spectral.npz/
geometric_tilt.npz back in at all -- CreatePage's spectral card (see
below) only gates "Continue" on a spectral calibration having been
*created* this session, it does not feed one into the resulting
CalibrationBundle; CalibrationBundle.wavelength_axis stays None even
after a real spectral calibration exists on disk (see its own docstring).
...
class CalibrationBundle:
    ...
    wavelength_axis
        Always None for now -- SpectralCalibrationDialog's accept path
        does build and save a real WavelengthCalibrationResult (see
        calibration_dialogs.py's module docstring), but
        _attempt_load_existing_calibrations() does not read spectral.npz
        back in to populate this field (see its own docstring); a
        follow-up pass can wire that in once a caller actually needs a
        real wavelength axis out of this screen.
